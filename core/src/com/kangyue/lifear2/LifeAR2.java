...
public class LifeAR2 implements ApplicationListener {
	 ...
	 */
	public LifeAR2(DeviceCameraControl cameraControl) {
	...
	public void create() {
		...
		button_camera = new Button(style);
		// 设置按钮的位置
		button_camera.setPosition(50, 300);
		// 给按钮添加点击监听器
		button_camera.addListener(preview_on);
		...
		if (mode == Mode.waitForPictureReady) {
			...
			if (deviceCameraControl.getPictureData() != null) {
				...
				FileHandle jpgfile_screenshot = Gdx.files
						.external("libGdxSnapshot" + "_" + date
								+ "_screenshot.jpg");
